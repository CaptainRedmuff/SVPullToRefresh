...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler
{
    ...
    if (!self.pullToRefreshView)
    {
        SVPullToRefreshView *view
            = [[SVPullToRefreshView alloc] initWithFrame:CGRectMake(0, -SVPullToRefreshViewHeight, self.bounds.size.width, SVPullToRefreshViewHeight)];
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.titles = [NSMutableArray
            arrayWithObjects:NSLocalizedString(@"Pull to refresh...", ), NSLocalizedString(@"Release to refresh...", ), NSLocalizedString(@"Loading...", ), nil];
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    if (self.superview && newSuperview == nil)
    {
        ...
        if (scrollView.showsPullToRefresh)
        {
            if (self.isObserving)
            {
                // If enter this branch, it is the moment just before "SVPullToRefreshView's dealloc", so remove
                // observer here
...
- (void)layoutSubviews
{
    ...
    CGRect boundingRect = [self.titleLabel.text boundingRectWithSize:self.bounds.size
    ...
    titleFrame.size.width = boundingRect.size.width;
    titleFrame.size.height = boundingRect.size.height;
...
- (void)setCustomView:(UIView *)view forState:(SVPullToRefreshState)state
{
    ...
    if (state == SVPullToRefreshStateAll)
        [self.viewForState replaceObjectsInRange:NSMakeRange(0, 3)
                            withObjectsFromArray:@[ viewPlaceholder, viewPlaceholder, viewPlaceholder ]];
    else
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate
{
    ...
    self.dateLabel.text
        = [NSString stringWithFormat:NSLocalizedString(@"Last Updated: %@", ),
                                     newLastUpdatedDate ? [self.dateFormatter stringFromDate:newLastUpdatedDate] : NSLocalizedString(@"Never", )];
}
...
- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter
{
    ...
    self.dateLabel.text
        = [NSString stringWithFormat:NSLocalizedString(@"Last Updated: %@", ),
                                     self.lastUpdatedDate ? [newDateFormatter stringFromDate:self.lastUpdatedDate] : NSLocalizedString(@"Never", )];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5)
    {
        NSArray *alphaGradientColors
            = [NSArray arrayWithObjects:(id)[self.arrowColor colorWithAlphaComponent : 0].CGColor, (id)[self.arrowColor colorWithAlphaComponent : 1].CGColor, nil];
